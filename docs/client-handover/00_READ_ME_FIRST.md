...
| Product image upload endpoint | Does not check the admin login token on the server side. Recommended to fix before or shortly after handover. See "Future Recommendations, Urgent". *(Verified from code)* |
